...
if command == "get" then
    ...
    if not youtube_url then
        ...
        local response_body_json, status_code = sendRequest("POST", "/convert", { youtube_url = youtube_url }) -- Now expects JSON table back
        if response_body_json then
            ...
            print("Use 'status <process_id>' to check progress, where process_id is typically the video ID.")
             ...
             if response_body_json.status then
                  print("Server Response Status: " .. tostring(response_body_json.status))
                  if response_body_json.message then print("Server Message: " .. tostring(response_body_json.message)) end
                  if response_body_json.process_id then print("Server Process ID: " .. tostring(response_body_json.process_id)) end
             else
                  printError("Server returned JSON without a 'status' key.")
             end

        elseif status_code then
             -- sendRequest returned nil for body but a status code, likely failed JSON decode
             printError("Received status code " .. tostring(status_code) .. " but server response was not valid JSON.")
        else
            -- sendRequest returned nil for both, likely a connection or pcall error already printed
    ...
    if not tape_side_arg or not youtube_url then
        ...
        if tape_side then
            print("Requesting conversion and writing for: " .. youtube_url)
             ...
             local dfpwm_data, status_code = sendRequest("POST", "/convert_and_get", { youtube_url = youtube_url })
             if dfpwm_data then
             ...
             elseif status_code then
                 -- Check if it was an error status code or just failed to get data
                 if status_code >= 200 and status_code < 300 then
                      printError("Received status code " .. tostring(status_code) .. " but no DFPWM data was received.")
                 else
                      -- Error already printed by sendRequest for non-2xx status
                      printError("Failed to retrieve DFPWM data.")
                 end
             else
                  -- Error already printed by sendRequest for connection/pcall failure
                  printError("Failed to get a valid response from server.")
             end
        end
    end
...
elseif command == "list" then
    ...
    local file_list, status_code = sendRequest("GET", "/list_files", nil) -- Expects JSON table back
    if file_list then
        if type(file_list) == "table" then
            ...
            printError("Failed to parse file list or list is not a table.")
             printError("Raw response may be in debug logs above.")
        end
    elseif status_code then
         printError("Received status code " .. tostring(status_code) .. " but server response was not valid JSON file list.")
    else
         printError("Failed to get a valid response from server.")
    end
...
elseif command == "status" then
     ...
     if not process_id then
         ...
         local status_info, status_code = sendRequest("GET", "/status/" .. textutils.urlEncode(tostring(process_id)), nil) -- URL encode process_id, expects JSON table
         if status_info then
              if type(status_info) == "table" then
                  print("\nProcess Status:")
         ...
         elseif status_code then
              printError("Received status code " .. tostring(status_code) .. " but server response was not valid JSON status info.")
         else
              printError("Failed to get a valid response from server.")
         end
     end
...
elseif command == "download" then
    ...
    if not filename then
        ...
        local dfpwm_data, status_code = sendRequest("GET", "/download/" .. textutils.urlEncode(tostring(filename)), nil) -- URL encode filename
        if dfpwm_data then
        ...
        elseif status_code then
             printError("Received status code " .. tostring(status_code) .. " but no data was received for download.")
        else
            printError("Failed to get a valid response from server.")
        end
    end
